...
if (count($dataArray) == 1) {
    if ($dataArray[0] == "semua") {
        $status = 0;
        if ($sql['level'] == "semua") {
            $data = mysqli_query($koneksi, "SELECT id_kelas FROM siswa where ruang='$ruang' and sesi='$sesi' group by id_kelas");
        } else {
            $tingkat = $sql['level'];

            $data = mysqli_query($koneksi, "SELECT id_kelas FROM siswa WHERE ruang='$ruang' and sesi='$sesi' and level='$tingkat' group by id_kelas");
        }
