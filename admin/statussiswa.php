...
if ($pengawas['level'] == 'pengawas' || $pengawas['level'] == 'proktor') {
	$nilaiq = mysqli_query($koneksi, "SELECT s.id_nilai, s.id_ujian, s.id_mapel, s.id_siswa, s.kode_ujian, s.ujian_mulai, s.ujian_berlangsung, s.ujian_selesai, s.jml_benar, s.jml_salah, s.nilai_esai, s.skor, s.total, s.status, s.ipaddress, s.hasil, s.jawaban, s.jawaban_esai, s.nilai_esai2, s.online, s.id_soal, s.id_opsi, s.id_esai, c.id_ujian, c.kode_nama, c.id_pk, c.id_guru, c.kode_ujian, c.nama, c.jml_soal, c.jml_esai, c.bobot_pg FROM nilai  s LEFT JOIN ujian c ON s.id_ujian=c.id_ujian JOIN siswa b ON b.id_siswa=s.id_siswa where c.status='1' and s.id_siswa<>'' and b.ruang='$pengawas[ruang]' GROUP by s.id_nilai ORDER BY c.nama DESC");
} else {
