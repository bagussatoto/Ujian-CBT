...
if ($cek > 0) :
    ...
    if ($exec) {
        $exec = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM mapel WHERE kode='". $kode ."'"));
        
        $sql = "INSERT INTO soal(id_mapel,nomor,soal,jenis,pilA,pilB,pilC,pilD,pilE,jawaban,file,file1,fileA,fileB,fileC,fileD,fileE) SELECT ". $exec['id_mapel'] .",nomor,soal,jenis,pilA,pilB,pilC,pilD,pilE,jawaban,file,file1,fileA,fileB,fileC,fileD,fileE FROM soal WHERE id_mapel=". $id;

        $exec = mysqli_query($koneksi, $sql);
        if ($exec) {
            echo "OK";
        } else {
            echo "Gagal menyalin soal. Berhasil Tambah Bank Soal.";
        }
    }
endif;
